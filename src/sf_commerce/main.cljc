(ns sf_commerce.main
  "Kotodama WASM entrypoint for the Sf-commerce clean-room actor (L4) — Clojure port.
  ...
  facts (`sf_commerce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sf_commerce")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "sfcommer_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "sfcommer_cus"
    :fields [:email :firstName :lastName :phone]          :required [:email :firstName]
    :coerce {}                                 :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "sfcommer_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"sf_commerce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sf-commerce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sf-commerce-compat/ready)
